...
**BEL `be` key-parity harden (2026-09-16):** ~**43.9 / 50** — verified **368 / 368** message keys vs `ru`/`uk`; remaining RU-identical loading chrome rewritten (adult BE); concept-label typo `Прадстаўленне`; ADR-005 + data-model list `be`; unit parity guard. Locale switcher / middleware `/be` / onboarding L1→`be` skim **OK**. **No Neon/Vercel login.** Still **WAITING_ON_FOUNDER**.
...
### Critical re-score (post Droga i transport substance deepen) — `/50`

| # | Criterion | before | after | Note |
| --- | --- | ---: | ---: | --- |
| 1 | Session-1 life loop | 4.5 | 4.5 | Unchanged — durable host still EXTERNAL |
| 2 | Feedback that teaches | 4.65 | 4.75 | Droga L01–L03 distractors + distinct L1 + triple gaps |
...
| | **Total** | **~42.9** | **~43.0** | CODE substance; goal still EXTERNAL |

### Critical re-score (post W sklepie substance deepen) — `/50` (historical)
...
| --- | --- | ---: | ---: | --- |
| 1 | Session-1 life loop | 4.5 | 4.5 | Unchanged — durable host still EXTERNAL |
| 2 | Feedback that teaches | 4.55 | 4.55 | Unchanged |
| 3 | Listening + speaking path | 4.0 | 4.1 | Adult try-first speaking + listening idle/error/retry; still interim TTS/Web Speech |
| 4 | Human-readable progress | 4.1 | 4.1 | Unchanged |
| 5 | Zero civilian ops jargon | 4.35 | 4.35 | Unchanged |
| 6 | BEL first-class L1 | 5.0 | 5.0 | Unchanged |
| 7 | Content honesty contract | 4.0 | 4.0 | Still DRAFT; no JPJO |
| 8 | Deploy teaches someone | 4.45 | 4.45 | No Neon/Vercel; agents must not force login |
| 9 | Admin ≤ learner craft | 3.55 | 3.8 | Freeze held; learner craft shipped; no `/admin` nav leak |
| 10 | Competitive first week | 3.85 | 3.85 | Still PARTIAL_TUNNEL |
| | **Total** | **~42.4** | **~42.7** | §9 CODE lift; goal still EXTERNAL |

### Critical re-score (ruthless independent 2026-09-16) — `/50` (historical)

| # | Criterion | before (claimed) | after | Note |
| --- | --- | ---: | ---: | --- |
| 1 | Session-1 life loop | 4.65 | 4.5 | LIVE_TUNNEL proven; host babysitting ≠ staff-free durable |
| 2 | Feedback that teaches | 4.65 | 4.55 | A1 chrome solid; not deep tutoring |
| 3 | Listening + speaking path | 4.0 | 4.0 | edge-tts + Web Speech interim ceiling |
| 4 | Human-readable progress | 4.1 | 4.1 | Labels/CTAs OK; durable multi-day EXTERNAL; `кlatka` fixed |
| 5 | Zero civilian ops jargon | 4.3 | 4.35 | Key+value guard; mastery/triage keys renamed |
| 6 | BEL first-class L1 | 5.0 | 5.0 | `be` locale + parity held |
| 7 | Content honesty contract | 4.0 | 4.0 | Still DRAFT; no JPJO |
| 8 | Deploy teaches someone | 4.7 | 4.45 | Docs/scripts real; no durable smoke — do not inflate docs |
| 9 | Admin ≤ learner craft | 3.5 | 3.55 | Freeze; SiteHeader has no `/admin` for learners |
| 10 | Competitive first week | 4.0 | 3.85 | PARTIAL_TUNNEL only; phrasebot wins friction |
| | **Total** | **~44.3** | **~42.4** | Honesty deflation; goal still EXTERNAL |

### Critical re-score (post §5 civilian jargon sweep) — `/50` (historical)

| # | Criterion | before | after | Note |
| --- | --- | ---: | ---: | --- |
| 1 | Session-1 life loop | 4.65 | 4.65 | Unchanged — durable host still EXTERNAL |
| 2 | Feedback that teaches | 4.65 | 4.65 | Unchanged |
| 3 | Listening + speaking path | 4.0 | 4.0 | Unchanged |
| 4 | Human-readable progress | 4.1 | 4.1 | Unchanged |
| 5 | Zero civilian ops jargon | 3.95 | 4.3 | Residual DEMO_/path dumps/освоение/DRAFT labels gone; JPJO guard; banner stays honest |
| 6 | BEL first-class L1 | 5.0 | 5.0 | Unchanged |
| 7 | Content honesty contract | 4.0 | 4.0 | Still DRAFT; no JPJO |
...
| | **Total** | **~43.9** | **~44.3** | §5 CODE lift; goal still EXTERNAL |

### Critical re-score (post BEL key-parity harden) — `/50` (historical)
...
| 6 | BEL first-class L1 | 4.9 | 5.0 | Key parity + RU loading remnants gone; still not JPJO-reviewed |
| 7 | Content honesty contract | 4.0 | 4.0 | Still DRAFT; no JPJO |
| 8 | Deploy teaches someone | 4.7 | 4.7 | No Neon/Vercel; agents must not force login |
| 9 | Admin ≤ learner craft | 3.5 | 3.5 | Freeze held |
| 10 | Competitive first week | 4.0 | 4.0 | Unchanged — still PARTIAL_TUNNEL |
| | **Total** | **~43.8** | **~43.9** | Micro CODE lift; goal still EXTERNAL |

### Critical re-score (post BEL UI locale) — `/50` (historical)
...
**Status:** ~**43.0 / 50** — Droga + W sklepie substance deepen + prior §9 craft; LIVE_TUNNEL day-1 still **done**. Goal still **EXTERNAL**.
...
Score estimate ~**43.0 / 50** (post droga substance; prior sklep ~42.9). No fake PUBLISHED. No Neon claimed. LIVE_TUNNEL day-1 **PASS** (ephemeral). BEL learners can use `/be/…` menu. See **WAITING_ON_FOUNDER** above — durable Neon+Vercel **and** JPJO; agents must not force logins.
