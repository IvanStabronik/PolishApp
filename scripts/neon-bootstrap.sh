...
echo ""
echo "  Preferred (founder email + password, DEMO_MODE stays false):"
echo ""
echo "    bash $REPO_ROOT/scripts/bootstrap-first-admin.sh \\"
echo '      --database-url "<same DIRECT URL>" --email "you@example.com" --password "<strong>"'
echo ""
echo "  Fallback (demo accounts via FORCE_SEED — laptop only; DEMO_MODE unset/false):"
echo ""
echo "    cd $WEB_DIR"
...
echo "    - Sign in as the bootstrap admin (or DEMO_ACCOUNTS.admin if FORCE_SEED fallback)"
